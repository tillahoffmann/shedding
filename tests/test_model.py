...
def test_vector_values_roundtrip(batch):
    ...
    values = {
        key: np.random.normal(size=batch + shape) for key, shape in parameters.items()
    }
    ...
    [
        np.testing.assert_allclose(value, values[key])
...
@pytest.fixture(
    params=it.product(shedding.Parametrisation, [False, True], shedding.Profile)
)
...
def test_transform(model: shedding.Model):
    ...
    original = {
        key: np.random.normal(0, 1, value) for key, value in model.parameters.items()
    }
